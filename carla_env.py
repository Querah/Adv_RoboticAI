...
from env_utils.basic_agent import BasicAgent as Agent
from env_utils.local_planner import LocalPlanner
...
class Weather(object):
    def __init__(self, world, changing_weather_speed, rain=80.0, sun_altitude=90.0):
        self.world = world
        ...
        self._sun = Sun(self.weather.sun_azimuth_angle, altitude=sun_altitude)
        self._storm = Storm(precipitation=rain)


    def reset(self):
        self.weather.precipitation = self.rain
        self.weather.sun_altitude_angle = self.sun_altitude
...
class CarlaEnv(object):
    ...
    def __init__(self,
        ...
        # weather
        self.weather = Weather(self.world, changing_weather_speed=self.changing_weather_speed,
                               rain=self.precipitation, sun_altitude=self.sun_altitude)
        self.weather.tick()
    ...
    def reset_vehicle(self):
        ...
        # start_location = carla.Location(x=138.848526, y=-171.748428, z=0.1)
        # start_rotation = carla.Rotation(pitch=0.000000, yaw=1.000204, roll=0.000000)
        self.vehicle_start_pose = carla.Transform(carla.Location(x=start_x, y=0, z=0.1), carla.Rotation(yaw=-90))
        # self.vehicle_start_pose = carla.Transform(start_location,start_rotation)
